...
**Two different tokens — use the right one**

There are *two* tokens in play, and they are not interchangeable:

| Token | Where it comes from | What it has |
|---|---|---|
| **Graph API Explorer token** | Explorer, with your app selected | Your user + the scopes you toggled. Selecting the app does **not** add your user to the sandbox ad account, so it can hit `(#200)` on `act_...`. |
| **Key-icon token** | The 🔑 next to the sandbox account in **Marketing API → Tools** | Generated by the sandbox tooling and already bound to that sandbox ad account with `ads_management`/`ads_read`. **This is the token to use for sandbox calls.** |

Them being different is expected. Set `ACCESS_TOKEN` to the **key-icon token**, then
confirm both the account and the Page resolve on that one token (see the `--page` check
below) — publishing an ad needs a single token that holds **both** the sandbox account
**and** an `ADVERTISE` role on the Page.

**Fix — reconnect the sandbox account to your token (in order)**

1. **Use the key-icon token (or the same app).** Prefer the 🔑 token next to the sandbox
   account in **Marketing API → Tools** — it is pre-bound to the account. If you instead
   use a [Graph API Explorer](https://developers.facebook.com/tools/explorer) token, pick
   the **same app that owns the sandbox account** in the top-right dropdown; a token from a
   different app will never see this account.
...
5. **Re-run the preflight** until both `account_access` and `page_access` are `OK`.
   Pass the Page id (or set `PAGE_ID`) so doctor checks the Page on the *same* token:

   ```bash
   python -m meta_ads_pipeline doctor --live \
     --account "$SANDBOX_AD_ACCOUNT_ID" \
     --page 1074742885732865
   ```
...
enough, and the `doctor` `page_access` check confirms exactly this. The Page does **not**
need to be "owned" by the sandbox ad account; it only needs to be usable by the **same
token** that holds the sandbox account. Put that Page id in `Creatives.page_id` (or
`Accounts.page_id`) so every creative and ad references it.

> ⚠️ The catch: the key-icon token gives you the **account**, and your Page role gives you
> the **Page** — but you need *one* token with both. Run the `doctor` command above; if
> `account_access` is `OK` and `page_access` `FAIL`s (or vice-versa), you're holding the
> wrong token for one of them. The token's user must manage the Page *and* hold an ads
> role on the sandbox account.
